...
@registry.register_reward("dual_reward")
class DualReward(BaseReward):
    def __init__(
        self,
    ...
    def _reshape_like_groups(self, flat_items, group_sizes):
        return reshape_like_groups(flat_items, group_sizes, caller_name="DualReward")
    ...
    def score(self, samples, responses, reward_metas):
        ...
        if len(valid_think_texts) > 0:
            try:
                ...
                if should_reraise_extractor_error(error):
                    raise
                log_extractor_warning("DualReward", len(valid_think_texts), error)
                if not self.zero_on_extract_failure:
                    raise
                extracted_openset_texts = [""] * len(valid_think_texts)
        ...
        reward_info = {
            "type": "dual",
            "score_type": "f1",
            "valid_think_mask": self._reshape_like_groups(flat_valid_mask, group_sizes),
            "think_texts": self._reshape_like_groups(flat_think_texts, group_sizes),
            "think_openset_texts": self._reshape_like_groups(flat_pred_texts, group_sizes),
            "think_openset_lists": self._reshape_like_groups(flat_pred_labels, group_sizes),
            ...
            "think_lengths": self._reshape_like_groups(flat_pred_lengths, group_sizes),
